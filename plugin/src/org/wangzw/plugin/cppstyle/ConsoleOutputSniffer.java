...
/**
 * Intercepts an output to console and forwards it to console parsers for
 * processing
 */
public class ConsoleOutputSniffer {
	...
	private class ConsoleOutputStream extends OutputStream {
		...
		/**
		 * Checks to see if the already read input constitutes a complete line
		 * (e.g. does the sniffing). If so, then send it to processLine.
		 *
		 * @param flush
		 */
		private void checkLine(boolean flush) {
			...
			while ((i = buffer.indexOf('\n')) != -1) {
				...
				if (i > 0 && buffer.charAt(i - 1) == '\r') {
					// also get rid of trailing \r in case of Windows line
					// delimiter "\r\n"
					eol = i - 1;
				}
	...
	/**
	 * Returns an output stream that will be sniffed. This stream should be
	 * hooked up so the command output stream goes into here.
	 */
	...
	/**
	 * Returns an error stream that will be sniffed. This stream should be
	 * hooked up so the command error stream goes into here.
	 */
		...
		if (nOpens > 0 && --nOpens == 0) {
			for (int i = 0; i < parsers.length; ++i) {
				try {
					...
					// Report exception if any but let all the parsers a chance
					// to shutdown.
					CppStyle.log(e);
				}
			}
	...
	private synchronized void processLine(String line) {
		for (IConsoleParser parser : parsers) {
			try {
				// Report exception if any but let all the parsers a chance to
				// process the line.
